...
while (g<=6) {
  ...
  if(k===4 || g===7){
    ...
    var groupImage = document.getElementsByName('group'+g+'shown_'+k)
    var select = document.getElementById('group'+g+'_sign_image_'+k)
    var textbox = document.getElementById('group'+g+'_sign_text_'+k)
      for (var i = 0; i < groupImage.length; i++) {
        groupImage[i].addEventListener('change', function() {
          if (this.checked) {
            setImage('group'+g+'_sign', 'group'+g+'_sign', this.value, k);
          }
        })
      }
      select.addEventListener('change', (function(g, k) {
        return function () { setImage('group'+g+'_sign', 'group'+g+'_sign', 'image', k) }
      } (g, k)))

      textbox.addEventListener('keyup', (function(g, k) {
        return function () { setImage('group'+g+'_sign', 'group'+g+'_sign', 'text', k) }
      } (g, k)))
...
function setImage(layer, svg, type, number, other) {
  var image = document.getElementById(layer+'_image_'+number);
  var text = document.getElementById(layer+'_text_'+number);

//reset     group3_sign_image_1
  if (document.getElementById(svg+'group_'+number)) {
    var group = document.getElementById(svg+'group_'+number);
  ...
  if (type === 'image') {
    ...
    g.setAttributeNS(null, "id", svg+'group_'+number);
    if(other) {
      g.setAttributeNS(null, "style", other);
    }
    document.getElementById(svg+'_'+number).appendChild(g)
    ...
    if (image.value === 'flagger') {
      ...
      document.getElementById(svg+'group_'+number).appendChild(s)
      s = null
    ...
    textbox.setAttributeNS(null, "id", svg+'group_'+number);
    ...
    document.getElementById(svg+'_'+number).appendChild(textbox)
